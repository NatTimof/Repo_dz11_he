...
class Birthday(Field):
    ...
    def value(self, value):
        if value is not None:
            try:
                date_of_birth_datetime = datetime.strptime(value, '%d.%m.%Y').date()
                self.__value = date_of_birth_datetime
            except ValueError:
...
class Record:
    ...
    def days_to_birthday(self):
        if self.birthday.value:
            today = date.today()
            birthday_current_year = self.birthday.value.replace(year=today.year)

            if today > birthday_current_year:
                birthday_next_year = self.birthday.value.replace(year=today.year + 1)
